...
public class EmployeeController {
	...
	public String empNSPrint(HttpServletRequest request, Model model) {
		...
		if(requestno.length == code.length && requestno.length == name_class_of_child.length && requestno.length == amount_claimed.length && requestno.length == amount_approved.length) {
			...
			for(int i=0; i<requestno.length;i++) {
				...
				Optional<Employee_allowance> empAllowance = employee_allowanceRepo.findById(Integer.parseInt(requestno[i]));
				...
				if(empAllowance.isPresent()) {
					if(empAllowance.get().getNoofchilds().equalsIgnoreCase("1")) {
						if(Integer.parseInt(amount_approved[i]) <= empAllowance.get().getAmount_claimed()) {
							...
							model.addAttribute("msg", requestno[i]+" Request Number Approved Amount should not more than Amount Claimed");
							return "empNSForm";
						}
					}
			...
			reqList.stream().forEach(a ->  listEmpAllowances.add(employee_allowanceRepo.findById(Integer.parseInt(a)).get()));
	...
	public String empSOPrint(HttpServletRequest request, Model model) {
		...
		System.out.println("requestNo==> "+request.getParameter("requestno"));
		System.out.println("so_noList==> "+request.getParameter("so_noList"));
		...
		if(requestno.length == so_no.length) {
			...
			for(int i=0; i<requestno.length;i++) {
				...
				if(empAllowance.isPresent()) {
						empAllowance.get().setSo_no(so_no[i]);
					 model.addAttribute("SONumber", so_no[i]);
