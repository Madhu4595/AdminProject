...
function searchrequestno() {
	...
	if (requestno === "" || requestno === null) {
		...
		$.ajax({
			type : "get",
			url : "./getGemVehicleForNS?requestno="+requestno,
			cache : false,
			success : function(response) {
				//alert("success response length=> "+response.length)
				//alert("success response  => "+JSON.stringify(response))
				...
				if(response.length === 0){
					...
					document.forms[0].action="./generateGEMVehicleNoteSheet";
...
<%@include file="banner2.jsp"%>
	...
	<div align="center">
		...
		<div class="container mt-2 mb-4 border font-weight-bolder"
